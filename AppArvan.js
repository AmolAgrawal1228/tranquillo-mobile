...
function HomeScreen({ navigation }) {
  ...
  function pythonTran(txt,lang) {
    return new Promise((resolve, reject) => {
      fetch("http://192.168.86.51:5000/translate", {
      method: 'PUT',
      headers : {
    ...
    .then(function(data){
      console.log(data[lang][txt])
      resolve(data[lang][txt])
    }) 
  ...
  return (
    <ImageBackground source={backgroundImage} resizeMode="cover">
      <View>
        ...
        <View style={styleTest}>
          <Button
            onPress={() => pythonTran("i love you","Arabic").then(value => {Alert.alert(value);})}
            title="Resources"
            color="#0278F5"
